...
const LinePresetSchema = z.object({
  ...
  chapterArcDefaults: z.object({
    ...
    shameEscalationStart: z.number().int().min(1).max(15),
    revengeActivationChapter: z.number().int().min(1).max(15),
    dignityRecoveryChapter: z.number().int().min(1).max(15),
  }),
  ...
  constraints: z.object({
    fixedChapterCount: z.number().int().positive().default(15),
