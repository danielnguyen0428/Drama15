...
export function buildStoryBiblePrompt(params: {
  ...
  return {
    ...
    userPrompt: [
      ...
      "premise, heroine, betrayer, rival, classHierarchy, supportingPressureCast, betrayalEngine, classShameEngine, revengeEngine, endingMode, pressureThreads.",
      ...
      "supportingPressureCast is an array of named secondary characters who apply pressure on the heroine beyond the betrayer and rival — parents, elders, guardians, in-laws, board members, a family council, an institution's gatekeeper. Each entry must include: name, role, relationshipToHeroine, pressureContribution.",
      "IMPORTANT: If the Concept (logline, promise, conflictEngine) or the premise implies a family, elder, guardian, council, or institutional force pressuring the heroine (e.g. 'the family forces the marriage', 'his mother opposes her', 'the board decides'), you MUST name that force explicitly as a supportingPressureCast entry. Never let a pressure source mentioned in the concept silently disappear from the bible. If the story is a genuine two-hander with no such force, return an empty array.",
      ...
      seedBlueprint ? renderSeedBlueprintForPrompt(seedBlueprint) : "",
      "pressureThreads is an array of the distinct pressure lines the story must resolve at the climax. List at least two threads, each a different KIND of pressure — for example a material/evidence/contract thread AND a social/emotional/dignity thread — not two versions of the same one. Each entry needs: label (short name of the thread) and resolutionBeat (one concrete sentence describing how chapter 14, the public climax, closes it). The climax must close every thread listed here, so do not list a thread you cannot resolve on-page.",
      customNicheLockInstruction(request),
      block("Request", request),
...
export function buildSettingSeedPrompt(params: {
  ...
  return {
    ...
    "Use realistic social conflicts and genre systems: family status pressure, workplace power, money, reputation, public shame, private dependence, social media, contracts, care labor, class-coded manners, pack law, mate-bond politics, alien captivity law, consent restoration, or empire contracts.",
      ...
      "The heroine's profession/trade MUST come from the castArchetype text verbatim (e.g. baker, mechanic, tutor, nurse, florist, auditor, hacker, translator). Do not substitute a speech therapist, physiotherapist, or any other therapist/rehab profession unless the word 'therapist' or 'physiotherapy' literally appears in the selected castArchetype below. This is a common overused default — actively avoid it when the castArchetype names a different trade.",
