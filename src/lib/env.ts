...
const EnvSchema = z.object({
  ...
  OPENAI_IMAGE_API_KEY: z
    .string()
    .optional()
    .transform((value) => value?.trim() || undefined),
  POSTER_IMAGE_MODEL: z.string().min(1).default("gpt-image-2"),
  POSTER_IMAGE_SIZE: z.string().min(1).default("1536x1024"),
  POSTER_IMAGE_QUALITY: z.string().min(1).default("high"),
  POSTER_IMAGE_TIMEOUT_MS: z.coerce.number().int().positive().default(300_000),
  POSTER_IMAGE_ENABLED: z
    .string()
    .optional()
    .transform((value) => value !== "false"),
  DEFAULT_LINE_PRESET: z.string().default("billionaire_rich_poor_romance"),
  DEFAULT_STYLE_PRESET: z.string().default("billionaire_rich_poor_romance__tiktok_hook_pacing"),
  DEFAULT_CHAPTER_COUNT: z.coerce.number().int().default(10),
  OUTPUT_DIR: z.string().optional(),
  WRITE_EXPORT_FILES: z
    .string()
    .optional()
...
export const env = {
  ...
  imageBaseUrl: (parsedEnv.OPENAI_IMAGE_BASE_URL || "https://api.openai.com/v1").replace(/\/+$/, ""),
  imageApiKey: parsedEnv.OPENAI_IMAGE_API_KEY || parsedEnv.OPENAI_API_KEY,
  posterImageModel: parsedEnv.POSTER_IMAGE_MODEL,
  posterImageSize: parsedEnv.POSTER_IMAGE_SIZE,
  posterImageQuality: parsedEnv.POSTER_IMAGE_QUALITY,
  posterImageTimeoutMs: parsedEnv.POSTER_IMAGE_TIMEOUT_MS,
  posterImageEnabled: parsedEnv.POSTER_IMAGE_ENABLED,
